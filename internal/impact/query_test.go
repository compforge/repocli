...
func TestAmbiguityIsQueryRelative(t *testing.T) {
	...
	if got.Scope != "focused" || len(got.TestFiles) != 0 || len(got.Observations) != 2 {
		t.Fatalf("%+v", got)
	}
	content["a.ts"] = "import {x} from './changed';"
	got = scoped(t, content, "changed.ts", ".")
	if got.Scope != "partial" || len(got.TestFiles) != 0 || got.Uncertainties[0].Reason != "ambiguous_import" {
		t.Fatalf("%+v", got)
	}
...
func TestKnownMembershipAcrossVersionsKeepsIssuesVisible(t *testing.T) {
	...
	if got.Scope != "focused" || !slices.Equal(got.TestFiles, []string{"test.test.ts"}) || len(got.Observations) != 1 || got.Observations[0].Version != "after" {
		t.Fatalf("%+v", got)
	}
}
