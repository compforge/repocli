...
`analysis` 协调这些能力，`impact` 负责变更起点、测试候选和结果筛选，`codegraph` 负责源码事实、
语言解析、局部构图及关系查询。gotreesitter 留在 CodeGraph 私有适配层，语法树和外部库对象不穿透到业务消费者。
...
每次 CLI 调用统一记录参数、版本、stdout/stderr、耗时、错误和退出状态，包括 help 与参数解析失败。
